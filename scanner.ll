%{
    ...
    #undef  YY_DECL
    #define YY_DECL int MyScanner::yylex( yy::MC_Parser::semantic_type * const lval, yy::MC_Parser::location_type *location )
    ...
    void print_data(bool flag, std::string name, std::string data, location_beta loc) {
    	if (flag) {
	    if (data != "-1") {
    ...
    void print_data(bool flag, std::string name, std::string data, yy::MC_Parser::location_type *location) {
	if (flag) {
	    if (data != "-1") {
...
{T_DIV} 	{ std::string symbol = "DIV"; std::string data = "-1"; print_data(1, symbol, data, loc); return Process(Tokens::T_DIV); }
...
{T_ID}		{ std::string symbol = "ID"; std::string data = YYText(); print_data(1, symbol, data, loc); return Process(Tokens::T_ID); }
{T_NUM} 	{ std::string symbol = "CONST"; std::string data = YYText(); print_data(1, symbol, data, loc); return Process(Tokens::T_NUM); }
